...
//
// Rules:
//   - Never overwrites a value that is already present (except a stale Prism
//     mock URL in NEXT_PUBLIC_API_URL, which is corrected to the real API).
...
const DOCKER_DATABASE_URL = "postgres://admin:password@localhost:5433/commerce_db";
...
function normalizeStorefrontApiUrl(filePath) {
  ...
  const normalized = lines.map((line) => {
    if (
      !changed &&
      new RegExp(`^\\s*NEXT_PUBLIC_API_URL\\s*=\\s*${escapeRegExp(PRISM_MOCK_URL)}\\s*$`).test(line)
    ) {
      changed = true;
...
function main() {
  ...
  ensureEnv(
    apiEnv,
    { DATABASE_URL: DOCKER_DATABASE_URL, JWT_SECRET: jwtSecret },
    [
  ...
  console.log("Done. Secrets are per-machine and never committed (see .gitignore).");
}
